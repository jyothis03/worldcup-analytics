    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content glass-panel animate-slide-up" onClick={e => e.stopPropagation()} style={{ padding: '32px' }}>
        ...
        <p style={{ textAlign: 'center', color: 'var(--text-secondary)', marginBottom: '32px' }}>
          {prediction?.home_team?.name || 'Home'} vs {prediction?.away_team?.name || 'Away'}
        </p>
        ...
        <div style={{ marginBottom: '40px', background: 'rgba(0,0,0,0.5)', padding: '24px', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '14px', fontFamily: 'Outfit' }}>
            <span style={{ color: 'var(--text-primary)' }}>{prediction?.home_team?.name || 'Home'}: {Math.round((prediction?.win_probability?.home || 0) * 100)}%</span>
            <span style={{ color: 'var(--text-muted)' }}>Draw: {Math.round((prediction?.win_probability?.draw || 0) * 100)}%</span>
            <span style={{ color: 'var(--text-primary)' }}>{prediction?.away_team?.name || 'Away'}: {Math.round((prediction?.win_probability?.away || 0) * 100)}%</span>
          </div>
          <div style={{ height: '8px', display: 'flex', borderRadius: '4px', overflow: 'hidden', background: 'rgba(255,255,255,0.05)' }}>
            <div style={{ width: `${(prediction?.win_probability?.home || 0) * 100}%`, background: 'var(--accent-neon)' }} />
            <div style={{ width: `${(prediction?.win_probability?.draw || 0) * 100}%`, background: 'var(--text-muted)' }} />
            <div style={{ width: `${(prediction?.win_probability?.away || 0) * 100}%`, background: 'var(--accent-dark)' }} />
          </div>
        </div>
            ...
            <div key={idx} style={{ background: 'rgba(0, 255, 136, 0.03)', padding: '20px', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                <p style={{ fontSize: '14px', marginTop: '4px' }}>
                  {Array.isArray(team?.key_players) ? team.key_players.join(', ') : (team?.key_players || 'None')}
                </p>
              </div>

              {Array.isArray(team?.injuries) && team.injuries.length > 0 && (
                <div>
        ...
        <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '24px' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', lineHeight: 1.6, fontSize: '15px' }}>{prediction?.reasoning || 'No analysis provided.'}</p>
        </div>
      </div>
    </div>
  );
